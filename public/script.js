...
function setHeaderProfiles(currentChat) {
  ...
  if (currentChat.participants.length > lengthToDisplay) {
    ...
    span.textContent = `+${currentChat.participants.length - lengthToDisplay}`;
...
function openMessageInfoModal(message) {
  ...
  document.getElementById("message-info-sender").textContent = message.username;
...
function setSocketEvents_responses() {
  socket.on("authenticated", async (user) => {
    ...
    document.getElementById("dash-iframe").src = `${rootUrl}/dashboard`;
  });
...
function mobileSwitchToPane(paneNumber) {
  ...
  if (paneNumber === 2) {
    ...
    history.pushState({ pane: 2 }, "Chat"); // Push state for chat pane
  } else {
    if (currentChat) {
      toHome();
    }
    ...
    history.pushState({ pane: 1 }, "Home"); // Push state for home pane
  }
}
...
function setHtmlContentListeners_mainChatPage() {
  ...
  document.getElementById("open-dash-btn").addEventListener(
    "click",
    async () => {
...
function setHtmlContentListeners_chatContainer() {
  ...
  document.addEventListener("paste", async (e) => {
    ...
    if (e.clipboardData.files.length > 0) {
      const file = e.clipboardData.files[0]; // Get the first file

      // Prevent the default paste behavior only if there is a file
      e.preventDefault();

      // Create a new DataTransfer object
      const dataTransfer = new DataTransfer();
      dataTransfer.items.add(file); // Add the pasted file to the DataTransfer object

      // Set the input's files property to the new FileList
      document.getElementById("file-chooser").files = dataTransfer.files;

      // Optionally, you can trigger any change event if needed
      const event = new Event("change", { bubbles: true });
      document.getElementById("file-chooser").dispatchEvent(event);

      console.log("File added to input:", file.name); // Log the added file
    }
  });
    ...
    () => {
      socket.emit(
        ...
        (user) => {
          ...
          showToast(
            `Account visibility set to ${user.private ? "Private" : "Public"}`,
          );
        },
      );
    },
  ...
  window.addEventListener("popstate", (event) => {
    if (isMobile) {
      if (event.state && event.state.pane) {
        const paneNumber = event.state.pane;
        mobileSwitchToPane(paneNumber); // Switch to the appropriate pane based on state
      }
    }
  });
